...
export function getMicroAppRouteComponent(opts) {
  ...
  const RouteComponent = {
    render(h) {
      ...
      // 默认取静态配置的 base
      let umiConfigBase = base ? (base === "/" ? "" : base) : "";
      console.log("getMicroAppRouteComponent === umiConfigBase", umiConfigBase);

      const extraPath = path.includes(umiConfigBase)
        ? path.split(umiConfigBase)[1]
        : path;
      ...
      const runtimeMatchedBase =
        umiConfigBase +
        (extraPath.endsWith("/")
          ? extraPath.substr(0, extraPath.length - 1)
          : extraPath);
      ...
      console.log(
        "getMicroAppRouteComponent === RouteComponent",
        componentProps
      );
